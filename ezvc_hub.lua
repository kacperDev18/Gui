...
lb(hdr,"v1.0",92,0,90,38,MT,9,false)
...
local function tg(p,nm,ds,x,y,w,h,on,cb)
    ...
    return {GetValue=function()return st end,SetValue=function(v)
        st=v
...
local function N(title,content)
    ...
    lb(nf,title,12,8,180,16,AC,11,true).ZIndex=51
    lb(nf,content,12,26,196,18,SB,10,false).ZIndex=51
...
local function killD(inst)
    ...
    if inst then if dlg then dlg:Destroy()end if scrD then scrD:Destroy()end dlg=nil scrD=nil return end
    ...
    task.delay(0.28,function()if dlg then dlg:Destroy()end if scrD then scrD:Destroy()end dlg=nil scrD=nil end)
end
local function ask()
    killD(true)
...
local function fB(tx)
    ...
    for _,r in ipairs(rr)do if r then for _,d in ipairs(r:GetDescendants())do
        if (d:IsA("TextButton")or d:IsA("ImageButton"))and vis(d)then
            local t=d.Text
            if d:IsA("ImageButton")then local lb2=d:FindFirstChildOfClass("TextLabel") if lb2 then t=lb2.Text end end
...
-- AUTO SKIP
local lastW=nil
local function getBtn() local o,b=pcall(function()return PG.MainGameUI.UpSide.InfoDop.AutoSkip end) return o and b or nil end
    ...
    mt.__namecall=newcclosure(function(self,...)
        ...
        if rec.active then pcall(function()
            ...
            elseif m=="InvokeServer" and SE and self==SE then
                ...
                if id then C.S=C.S+1;local tk=tick();rec.a[#rec.a+1]={t="S",i=id,d=tk-(rec.l or tk)};rec.l=tk;rec.t[id]=nil;rec.k[i]=nil;rfC() end
...
-- Override rfC to also update stats labels
local lobbyLbl=c1:GetChildren()[2] -- we'll do it properly:
-- rebuild with proper refs
local _lL=lb(c1,"",12,44,60,20,TX,16,true)
local _pL=lb(c1,"",82,44,60,20,TX,16,true)
local _uL=lb(c1,"",152,44,60,20,TX,16,true)
local _sL=lb(c1,"",222,44,60,20,TX,16,true)
rfC=function()
...
local mnBox=mk("TextBox",{Size=UDim2.new(1,-80,0,26),Position=UDim2.new(0,64,0,20),BackgroundColor3=BG,BorderSizePixel=0,Font=Enum.Font.Gotham,TextSize=11,TextColor3=TX,PlaceholderText="np. EasyFarm",PlaceholderColor3=MT,Text=Cfg.mn or "",ClearTextOnFocus=false},rc)
cr(mnBox,5)
mnBox.FocusLost:Connect(function() set("mn",mnBox.Text or "") end)

local rc2=cd(mP2,"Macro Actions",76)
rc2.Size=UDim2.new(1,-24,0,230)

-- Stats label in macro tab
local cL=lb(rc2,"Lobby 0 | Place 0 | Upgrade 0 | Sell 0",12,26,260,14,MT,9,false)
local _oldRfC=rfC
rfC=function()
    _oldRfC()
    cL.Text=string.format("Lobby %d | Place %d | Upgrade %d | Sell %d",C.L,C.P,C.U,C.S)
end

-- Saved macros dropdown (simple cycling)
local macroList=lf()
local macroIdx=0
for i,v in ipairs(macroList)do if v==Cfg.sel then macroIdx=i end end
local macroLbl=mk("TextButton",{Size=UDim2.new(1,-24,0,28),Position=UDim2.new(0,12,0,46),BackgroundColor3=EL,BorderSizePixel=0,Font=Enum.Font.Gotham,TextSize=11,TextColor3=TX,Text=Cfg.sel~="" and Cfg.sel or "-- select macro --",AutoButtonColor=false},rc2)
cr(macroLbl,5)
...
btn(rc2,"Create / Save Macro",12,82,240,28,"p",function()
    ...
    if n==""then N("EZVC","Wpisz nazwę macro") return end
