-- EZVC Hub v1.2 - Optimized + Safe Macro Recorder
...
-- Connection tracker (auto cleanup on unload)
local Conns={}
...
-- Accent refs (things that change color with theme)
local tRefs={}
...
lb(hdr,"v1.2",92,0,90,38,MT,9,false)
...
-- Record queue (anti-freeze: tower-detection spawn is throttled)
local placeQueue={}
...
-- __namecall hook (fast-path filter BEFORE any allocation)
pcall(function()
...
local themes={
    ...
    {n="Teal",  c=Color3.fromRGB(70,180,170)},
    {n="Lime",  c=Color3.fromRGB(160,210,90)},
}
...
task.spawn(function()
    task.wait(1.5)
    -- Apply saved accent to all refs (instant, no tween)
    for _,e in ipairs(tRefs) do
        local v=e.f and e.f() or AC
        e.p.BackgroundColor3=v
    end
