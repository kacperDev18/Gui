-- EZVC Hub v1.5 - Complete rewrite with fixed macro recorder
...
-- Config IO (before GUI so theme loads first)
local FO="tdmacro/"
...
lb(hdr,"v1.5",92,0,90,38,MT,9,false)
...
-- RECORD HOOK (FIXED)
-- ============================================================
-- Each PlaceTower spawns its OWN task with retry-until-found.
-- The tower is claimed atomically in rec.k upon discovery.
-- This prevents:
--   1) "only first unit saved" (bug from one-shot scan)
--   2) wrong ID → wrong tower mapping (kaskada bug)
--   3) overwriting previous actions
-- ============================================================
...
-- Retry-based tower finder: runs up to 3s, scans every 50ms,
-- claims first unclaimed tower within 20 studs of cf.Position.
...
pcall(function()
    ...
    mt.__namecall=newcclosure(function(self,...)
        ...
        local x={...}
        -- PlaceTower: fire remote, record immediately, spawn finder
        ...
        -- Other remotes: fire, record
        local r=old(self,...)
        if m=="InvokeServer" and self==HR.UP then
            local id=rec.k[x[1]]
            if id then
                C.U=C.U+1
                local tk=tick()
