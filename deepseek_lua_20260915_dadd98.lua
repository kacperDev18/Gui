-- EZVC Hub v1.3 - 8-color theme system
...
-- Accent refs: each entry is {p=instance, f=optional function, prop=property name}
local tRefs={}
local function rT(p,f,prop) tRefs[#tRefs+1]={p=p,f=f,prop=prop or "BackgroundColor3"} end
...
lb(hdr,"v1.3",92,0,90,38,MT,9,false)
...
-- Tabs (active tab text uses accent — reacts to theme change via rT)
...
local function go(n)
    ...
    for k,b in pairs(tabs) do
        if k==n then TS:Create(b,TweenInfo.new(0.22),{BackgroundColor3=EL,TextColor3=AC}):Play()
        else TS:Create(b,TweenInfo.new(0.22),{BackgroundColor3=SF,TextColor3=MT}):Play() end
    end
    cur=n
end
for i,n in ipairs({"Main","Play","Macro","GUI"}) do
    ...
    tabs[n]=b
    -- Active tab text color follows accent
...
for i,c in ipairs({Color3.fromRGB(155,120,255),Color3.fromRGB(108,142,255),Color3.fromRGB(0,200,220),Color3.fromRGB(90,196,140),Color3.fromRGB(220,96,96),Color3.fromRGB(240,150,60),Color3.fromRGB(230,120,180)}) do
    ...
    cr(s,6)
    ac(s.MouseButton1Click:Connect(function() setAC(c) end))
end
local function pg(n)
...
local function btn(p,t,x,y,w,h,k,cb)
    ...
    ac(o.MouseEnter:Connect(function() TS:Create(o,TweenInfo.new(0.14),{BackgroundColor3=bc:Lerp(Color3.new(1,1,1),0.15)}):Play() end))
    ac(o.MouseLeave:Connect(function() TS:Create(o,TweenInfo.new(0.14),{BackgroundColor3=bc}):Play() end))
...
local function setStatus(s)
    ...
    macroStatusLabel.TextColor3=col
end
...
rToggle=tg(rc2,"Record Macro","Start/stop recording",12,136,240,38,Cfg.rec,function(on)
    if on then
        if rec.active then
            N("EZVC","Already recording")
            rToggle.SetValue(true)
            return
        end
        if Cfg.pl then
            N("EZVC","Stop playback first")
...
-- ============================================================
-- GUI PAGE (8 colors: Purple, Red, Blue, Green, Orange, Pink, Cyan, Yellow)
-- ============================================================
local gP=pg("GUI")
local th=cd(gP,"Theme",12)
th.Size=UDim2.new(1,-24,0,204)
local themes={
...
for i,t in ipairs(themes) do
    ...
    local px=(col==0) and UDim2.new(0,12,0,30+row*36) or UDim2.new(0.5,6,0,30+row*36)
    local b=mk("TextButton",{Size=UDim2.new(0.5,-18,0,30),Position=px,BackgroundColor3=EL,BorderSizePixel=0,Font=Enum.Font.Gotham,TextSize=11,TextColor3=TX,Text="  "..t.n,AutoButtonColor=false,TextXAlignment=Enum.TextXAlignment.Left},th)
    cr(b,6)
    local dot=mk("Frame",{Size=UDim2.fromOffset(14,14),Position=UDim2.new(1,-22,0.5,-7),BackgroundColor3=t.c,BorderSizePixel=0},b)
    cr(dot,7)
    ac(b.MouseButton1Click:Connect(function()
        setAC(t.c)
        set("acr",t.c.R)
        set("acg",t.c.G)
        set("acb",t.c.B)
